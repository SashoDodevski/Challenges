...
<div class="flex flex-row justify-center pb-10 grow bg-gray-50">
    <div class="basis-2/3 text-left">
        ...
        <h6 class="text-2xl tracking-tight text-gray-900 dark:text-white text-center mb-2">Update donation:</h6>
        ...
        <div class="flex flex-col mx-auto mb-5 border rounded-lg w-full bg-gray-50 p-2 overflow-hidden overflow-y-auto p-6 shadow-lg lg:w-5/6">
            ...
            <form id="donationForm" class="mb-5" action="{{ route('donations.update', $donation) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                @enderror
                <input type="hidden" id="donation_id" value="{{old('donation_id', $donation->donation_id)}}" name="donation_id">

                <label for="donation" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"></label>
                <textarea rows="6" type="text" id="donation" name="donation" class="text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-red-500 dark:focus:border-red-500 @error('donation') bg-red-50 border border-red-500 text-red-900 placeholder-red-700 @else bg-gray-50 border border-gray-300 text-gray-900 @enderror" placeholder="Enter donation here...">{{old('donation', $donation->donation)}}</textarea>
                <button type="submit" id="donationFormBtn" class="w-20 text-center p-1.5 pl-5 pr-5 text-sm transition-colors duration-700 transform bg-lime-600 hover:bg-lime-500 text-gray-100 rounded-lg focus:border-4 border-green-300 my-5 ml-auto mr-0 hidden">Update</button>
            </form>
        </div>
    </div>
...
<script>
    $(function() {
        let updateDonationBtn = $("#updateDonationBtn");
