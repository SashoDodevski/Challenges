...
<div class="flex flex-row justify-center grow bg-gray-50">
    <div class="basis-2/3 text-center">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4">
            <table id="active" class="display w-full text-sm text-left text-gray-500 dark:text-gray-400">
...
<script>
    $(document).ready(function() {
        $('#active').DataTable({
            ...
            columnDefs: [{
                ...
                data: "download_link",
                render: function(id) {
                    return '<a href="contacts/' + id + '"><span class="font-semibold text-red-600 message" data-id="' + id + '">View</span></a>';
                }
            }]
        })
    })
    ...
    $("#active").on('click', '.message', function(event) {
        console.log('/contacts/' + event.target.getAttribute('data-id'))

        $.ajax({
            url: '/contacts/' + event.target.getAttribute('data-id'),
            method: 'PUT',
            data: {
