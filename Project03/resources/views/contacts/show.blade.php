    ...
    <div class="basis-2/3 text-left">
        ...
        <div class="flex flex-row justify-around">
            <div class="text-start">
                <a type="button" href="{{ route('contacts.index') }}" class="w-32 text-center p-1.5 pl-5 pr-5 text-sm transition-colors duration-700 transform bg-cyan-600 hover:bg-cyan-500 text-gray-100 rounded-lg focus:border-4 border-cyan-300 my-5 ml-0 mr-auto">Back</a>
            </div>
            <div class="text-start">
        ...
        <h6 class="text-2xl tracking-tight text-gray-900 dark:text-white text-center mb-2">Message:</h6>
        <div class="flex flex-col mx-auto mb-5 border rounded-lg w-full bg-gray-50 p-2 overflow-hidden overflow-y-auto p-6 shadow-lg lg:w-5/6">
            <div class="flex justify-between">
                <div class="text-start">
                    <h6 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white w-full text-start">{{ $contact->first_name . " " . $contact->last_name }}</h6>
                    <p class="font-normal text-gray-700 dark:text-gray-400 w-full"><span class="font-semibold">Email:</span> {{ $contact->email }}</p>
                    <p class="font-normal text-gray-700 dark:text-gray-400 w-full"><span class="font-semibold">Phone number:</span> {{ $contact->phone_number }}</p>
                </div>
            </div>

            <textarea disabled name="" id="" cols="30" rows="6" class="p-4 my-4 rounded-lg border-gray-300">{{ $contact->message }}</textarea>
        </div>
    </div>
